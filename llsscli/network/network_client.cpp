...
NetworkClient::NetworkClient(std::shared_ptr<boost::asio::io_service>& io_service,
                             std::shared_ptr<config::NetworkConfig>& config)
    : io_service_(*io_service.get()),
    ...
    try {
        ...
        boost::thread ClientThread(boost::bind(&boost::asio::io_service::run, &io_service_));
...
size_t NetworkClient::Open(std::shared_ptr<std::stringstream>& current_task) {

    return 0;
}

size_t NetworkClient::Close() {
    io_service_.post(boost::bind(&NetworkClient::DoClose_, this));
}
