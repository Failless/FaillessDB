...
Client::Client(config::ClientConfig& test_data) : config_(std::move(test_data)) {
    ...
    send_to_db_callback_ =
        std::make_shared<std::function<size_t()>>(std::bind(&Client::SendToDbCallback_, this));
    set_db_callback_ =
        std::make_shared<std::function<size_t()>>(std::bind(&Client::SetDBKeyCallback_, this));
    get_db_callback_ =
        std::make_shared<std::function<size_t()>>(std::bind(&Client::GetDBKeyCallback_, this));
    create_db_folder_callback_ = std::make_shared<std::function<size_t()>>(
        std::bind(&Client::CreateDBFolderCallback_, this));
    register_callback_ =
        std::make_shared<std::function<size_t()>>(std::bind(&Client::RegisterCallback_, this));
}
...
size_t Client::SetDBKey_() {
    ...
    // Init user Data struct
    value = std::make_unique<std::vector<unsigned char>>(query_tokens_[2].begin(),
                                                         query_tokens_[2].end());
