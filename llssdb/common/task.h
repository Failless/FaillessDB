...
class Data {
 public:
    ...
    explicit Data(short _folder_id = 0, size_t _size = 0, int8_t* _value = nullptr)
        : size(_size), value(_value), folder_id(_folder_id){};
...
class Task {
 public:
    Task() : query(nullptr), command(operators::GET) {}
    ...
    Task(short _id, size_t _size, std::string* _key, int8_t* _data, std::string* _query,
         operators _command, boost::uuids::uuid _client_id, boost::chrono::microseconds _time)
        : query(_query),
          payload(_id, _size, _data, _key),
          command(_command),
          client_id(_client_id),
          time(_time) {}
    explicit Task(std::string* _query, operators _command) : query(_query), command(_command) {}
    ...
    Task(int8_t* _data, size_t _size, std::string* _key,
         operators _command)  // this one is used for tests
